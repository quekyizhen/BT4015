...
r2 <- raster(pop_stats_npc, res=10000)
r2 <- rasterize(pop_stats_npc, field = Scl_P_D , r2)
...
#intersect buffer with npc polygon
buf_int_npc <- st_intersection(npc_point_buf, NPC_polygon_sf) 
...
hc_buf_intersection <- st_intersection(npc_point_buf, HC) #polygon, point
hc_buf_count <- as.data.frame(hc_buf_intersection %>% group_by(NPC_NAME) %>% count())[, 1:2]
names(hc_buf_count)[names(hc_buf_count) == 'n'] <- 'HC_in_buffer'

cc_buf_intersection <- st_intersection(npc_point_buf, CC) #polygon, point
cc_buf_count <- as.data.frame(cc_buf_intersection %>% group_by(NPC_NAME) %>% count())[, 1:2]
names(cc_buf_count)[names(cc_buf_count) == 'n'] <- 'CC_in_buffer'

ta_buf_intersection <- st_intersection(npc_point_buf, TA) #polygon, point
ta_buf_count <- as.data.frame(ta_buf_intersection %>% group_by(NPC_NAME) %>% count())[, 1:2]
names(ta_buf_count)[names(ta_buf_count) == 'n'] <- 'TA_in_buffer'

#colnames(npc@data) = col_names$column_names
